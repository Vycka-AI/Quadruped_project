...
model_save_path = "ppo_unitree_new_5.zip"
...
if os.path.exists(model_save_path):
    ...
    old_model_parameters = PPO.load(model_save_path, device='cpu').get_parameters()
    ...
    # Create the PPO model, passing in the custom architecture
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
        device='cpu',
        n_steps=2048,
        ent_coef=0.03,  # Add or adjust this parameter
        policy_kwargs=policy_kwargs  # Add the argument here
    )
    model.set_parameters(old_model_parameters)
